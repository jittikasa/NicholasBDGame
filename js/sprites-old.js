// Professional Sprite System - Using Nicholas's Aseprite Sprite Sheet
...
window.sprites = {
    nicholasSheet: null,
    cat: null,
    // Keep other sprites for compatibility
    bed: null,
    desk: null,
    giftbox: null,
    picnic: null
...
// Load Nicholas's professional sprite sheet
function loadSprites() {
    return new Promise((resolve, reject) => {
        console.log('🎨 Loading Nicholas professional sprite sheet...');
        ...
        nicholasImg.onload = () => {
            ...
            console.log('✅ Nicholas sprite sheet loaded successfully!');
            console.log(`📐 Sprite sheet size: ${nicholasImg.width}×${nicholasImg.height}px`);
        ...
        nicholasImg.onerror = () => {
            console.error('❌ Failed to load Nicholas sprite sheet');
            console.log('📁 Looking for: items/nicholas_spritesheet.png');
            console.log('🔄 Falling back to basic pixel art...');
            
            // Load picnic PNG even if Nicholas fails
            ...
            picnicImg.onerror = () => {
                console.error('❌ Failed to load picnic PNG too');
                window.allSpritesLoaded = true;
                resolve();
            };
        ...
        // Also try to load picnic independently
        setTimeout(() => {
            if (!window.sprites.picnic) {
                const picnicImg = new Image();
                picnicImg.onload = () => {
                    window.sprites.picnic = picnicImg;
                    console.log('✅ Picnic PNG loaded independently!');
                };
                picnicImg.onerror = () => {
                    console.log('📁 Picnic PNG not found, using FoMT fallback');
                };
                picnicImg.src = 'items/picnic.png';
            }
        }, 100);
